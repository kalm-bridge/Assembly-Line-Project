...
_start:
 ...
 movia  r15,0xfdbffbff		/* load threshold value HEX A for sensor 1 on lego controller*/
 stwio r15,0(r10)		        

 #sensor 1

 movia  r15,0xfdbfefff		/* load threshold value HEX A for sensor 2 on lego controller*/
 stwio r15,0(r10)	
...
wait_for_start:
 ...
 movia r10, instruction_flag
 ldb r10, 0(r10)
 beq r10, r0, wait_for_start
 call get_stored_input
 cmpeqi r9, r2, 0x001B
 beq r9, r0, wait_for_start*/

 movi r9, 0x01
 movia r10, start_cart
 stb r9, 0(r10)
...
main_motor_loop:
  ...
  bne r12, r0, follow_instructions
...
set_motors:
  ...
  ldbu r15, 0(r14)
  cmpeq r12, r15, r0
  bne r0, r12, check_crane_arm		#STOP CRANE

  cmpeqi r12, r15, 0x0074
  bne r0, r12, go_right
  cmpeqi r12, r15, 0x006b
  bne r0, r12, go_left
  br check_crane_arm
  go_left:
  movia r12, 0xffffffcf
  and r13, r13, r12
  br check_crane_arm

  go_right:
  movia r12, 0xffffffef
  and r13, r13, r12

  check_crane_arm:
  movia r14, storedInput4
  ldbu r15, 0(r14)
  cmpeqi r12, r15, 0x01
  bne r12, r0, close_claw
  movia r12, 0xffffffbf
  and r13, r13, r12
  br loadin
  close_claw:
  movia r12, 0xffffff3f
  and r13, r13, r12
  loadin:
  movia  r10, GPIO_JP1
  stwio r13, 0(r10)
...
check_crane2:
  ...
  ldbu r15, 0(r14)
  cmpeq r12, r15, r0
  bne r0, r12, check_crane_arm2		#STOP CRANE

  cmpeqi r12, r15, 0x0074
  bne r0, r12, go_right2
  cmpeqi r12, r15, 0x006b
  bne r0, r12, go_left2
  br check_crane_arm2
  go_left2:
...
HandleKeyBoard:
  ...
  is_enter:
   movia r10, Break_flag
   ldbu r9, 0(r10)
   bne r0, r9, is_break
   movi r9, 0x01
   stb r9, 0(r10) #look for break by setting flag
   movia r10, instruction_flag
   stb r9, 0(r10) #eat instruction
   br exit
   
   is_break:
   stb r0, 0(r10) #acknowledge break code
  br exit
  
  WriteCraneInput:
   movia r10, storedInput5
   cmpeqi r9, r12, 0x0029
   bne r9, r0, writezero
   stb r12, 0(r10)
   br exit
   writezero:
  ...
  WriteClawInput:
   movia r10, storedInput4
   cmpeqi r9, r12, 0x0075
   bne r9, r0, open_claw
   movi r9, 0x01
   stb r9, 0(r10)
   br exit
...
show_keyboard_input:
 ...
 DisplayOnHex:
  ...
  LOAD_DisplayClosed:
  ldbu  r11, 12(r5)    #load the 7-Seg format
  #stbio r11, 3(r2)           #store to 7-seg display
  br loadPattern
  ...
  cmpeqi r9, r10, 0x06b
  bne r9, r0, LOAD2_Display_left #if r9 = 1, then the current digit was a C
  cmpeqi r9, r10, 0x074
  bne r9, r0, LOAD2_Display_right
  
  LOAD2_Display0:
  ldbu  r11, 18(r5)    #load the 7-Seg format
  ldbu  r9, 19(r5)          #store to 7-seg display
  br loadPattern2
  
  LOAD2_Display_left:
  ldbu  r11, 17(r5)    #load the 7-Seg format
  ldbu  r9, 14(r5)          #store to 7-seg display
  br loadPattern2
  
  LOAD2_Display_right:
  ldbu  r11, 3(r5)    #load the 7-Seg format
  ldbu  r9, 17(r5)          #store to 7-seg display
  br loadPattern2
